...
#[cfg(test)]
mod tests {
    use super::{CLIENT_PREFACE, H2Decoder, H2Event};

    #[test]
    fn buffers_incomplete_frames_until_complete() {
        let mut decoder = H2Decoder::new();
        let frame = [
            0x00, 0x00, 0x11, 0x01, 0x04, 0x00, 0x00, 0x00, 0x01, 0x82, 0x86, 0x84, 0x41, 0x8c,
            0xf1, 0xe3, 0xc2, 0xe5, 0xf2, 0x3a, 0x6b, 0xa0, 0xab, 0x90, 0xf4, 0xff,
        ];
        assert!(
            decoder
                .push_client(&CLIENT_PREFACE[..10])
                .expect("preface-part")
                .is_empty()
        );
        assert!(
        ...
        assert!(
            decoder
                .push_client(&frame[..12])
                .expect("partial frame")
                .is_empty()
        );
        let events = decoder.push_client(&frame[12..]).expect("final frame");
        match &events[0] {
            H2Event::Headers(block) => {
                assert!(
                    block
                        .headers
                        .iter()
                        .any(|(name, value)| name == ":authority" && value == "www.example.com")
                );
            }
    ...
    #[test]
    fn decodes_padded_priority_headers_and_data() {
        let mut decoder = H2Decoder::new();
        let headers = [
            0x00, 0x00, 0x1a, 0x01, 0x2c, 0x00, 0x00, 0x00, 0x03, 0x03, 0x80, 0x00, 0x00, 0x01,
            0x10, 0x82, 0x86, 0x84, 0x41, 0x8c, 0xf1, 0xe3, 0xc2, 0xe5, 0xf2, 0x3a, 0x6b, 0xa0,
            0xab, 0x90, 0xf4, 0xff, 0x00, 0x00, 0x00,
        ];
        let data = [
            0x00, 0x00, 0x05, 0x00, 0x09, 0x00, 0x00, 0x00, 0x03, 0x01, b'a', b'b', b'c', 0x00,
        ];
        let _ = decoder.push_client(CLIENT_PREFACE).expect("preface");
        let mut events = decoder.push_client(&headers).expect("headers");
        events.extend(decoder.push_client(&data).expect("data"));
        match &events[0] {
            H2Event::Headers(block) => {
                let priority = block.priority.as_ref().expect("priority");
                assert_eq!(priority.stream_id, 3);
                assert_eq!(priority.dependency_stream_id, 1);
    ...
    #[test]
    fn decodes_dynamic_table_reset_and_goaway() {
        let mut decoder = H2Decoder::new();
        let indexed_header = [
            0x00, 0x00, 0x0c, 0x01, 0x05, 0x00, 0x00, 0x00, 0x01, 0x40, 0x04, b'h', b'o', b's',
            b't', 0x03, b'a', b'p', b'i', 0x82, 0x84,
        ];
        let indexed_followup = [
            0x00, 0x00, 0x03, 0x01, 0x05, 0x00, 0x00, 0x00, 0x03, 0xbe, 0x82, 0x84,
        ];
        let rst = [
            0x00, 0x00, 0x04, 0x03, 0x00, 0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00, 0x08,
        ];
        let goaway = [
            0x00, 0x00, 0x0b, 0x07, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x03, 0x00,
            0x00, 0x00, 0x00, b'b', b'y', b'e',
        ];

        let _ = decoder.push_client(CLIENT_PREFACE).expect("preface");
        let _ = decoder.push_client(&indexed_header).expect("header");
        let mut events = decoder.push_client(&indexed_followup).expect("followup");
        events.extend(decoder.push_client(&rst).expect("rst"));
        events.extend(decoder.push_client(&goaway).expect("goaway"));

        assert!(matches!(
            &events[0],
            H2Event::Headers(block)
                if block.headers.iter().any(|(name, value)| name == "host" && value == "api")
        ));
        assert!(matches!(
            &events[1],
            H2Event::RstStream {
